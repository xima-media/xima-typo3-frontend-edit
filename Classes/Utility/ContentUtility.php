...
{
    public static function fetchContentElements(
        int $pid,
        int $languageUid,
    ...
    public static function getTranslatedRecord(
        string $table,
        int $parendUid,
        int $languageUid
    ): array|bool {
        ...
        return $queryBuilder
            ...
            ->where(
                $queryBuilder->expr()->eq(
                    'l10n_parent',
                    $queryBuilder->createNamedParameter($parendUid, Connection::PARAM_INT)
                ),
                $queryBuilder->expr()->eq(
                    'sys_language_uid',
                    $queryBuilder->createNamedParameter($languageUid, Connection::PARAM_INT)
    ...
    public static function getContentElementConfig(string $cType, string $listType): array|bool
    {
        ...
        foreach ($tca as $item) {
            if (
                ($cType === 'list' && $item[$valueKey] === $listType) ||
                $item[$valueKey] === $cType
            ) {
    ...
    {
        $rootLine = GeneralUtility::makeInstance(RootlineUtility::class, $subPageId)
            ->get();
