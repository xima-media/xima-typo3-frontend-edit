...
# Function to set up the site configuration from the repo-owned templates in
# .ddev/.setup/templates/config/sites/ (config.yaml, settings.yaml, ...).
# This is the site's actual configuration - Site Set dependencies, languages,
# root page - overwriting whatever `typo3 setup` generated by default.
# Placeholders __VERSION__ and __SITENAME__ are replaced in every YAML file.
function setup_site_config() {
    local TEMPLATE_DIR="/var/www/html/.ddev/.setup/templates/config/sites"
    ...
    if [ "${MODE:-composer}" = "classic" ]; then
        TARGET_DIR="/var/www/html/.Build/$VERSION/public/typo3conf/sites"
    else
        TARGET_DIR="/var/www/html/.Build/$VERSION/config/sites"
    fi
    ...
    find "$TARGET_DIR" -name "*.yaml" -exec sed -i \
